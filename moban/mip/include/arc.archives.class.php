...
$this->Fields['body'] = str_replace('$', '$$', $this->Fields['body']);
	 ...
	 $this->Fields['body'] = preg_replace('/<img.*?src=[\"|\']?(.*?)[\"|\']?\s.*?>/i',"<mip-img data-carousel=\"carousel\" class=\"mip-element mip-img\" src=\"http://img.lypk.net$1\"></mip-img>", $this->Fields['body']);
     $this->Fields['body'] = preg_replace("(src=\"/uploads/allimg/)", "src=\"http://img.lypk.net/uploads/allimg/", $this->Fields['body']);
	 $this->Fields['body'] = preg_replace("(<a href=\"/)", "<a href=\"http://mip.lypk.net/", $this->Fields['body']);
     ...
     // 将"&nbsp;"替换为空格
     $this->Fields['body'] = preg_replace("/\&nbsp\;/i", " ", $this->Fields['body']);
     $this->Fields['body'] = preg_replace("/\&nbsp/i", " ", $this->Fields['body']);
    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                ...
                $this->PreNext['pre'] = "上一篇：<a target='_blank' href='$mlink'>{$preRow['title']}</a> ";
                $this->PreNext['preimg'] = "<a target='_blank' href='$mlink'><img src=\"{$preRow['litpic']}\" alt=\"{$preRow['title']}\"/></a> "; 
            }
            else
            {
            ...
            if(is_array($nextRow))
            {
                ...
                $this->PreNext['next'] = "下一篇：<a target='_blank' href='$mlink'>{$nextRow['title']}</a> ";
                $this->PreNext['nextimg'] = "<a target='_blank' href='$mlink'><img src=\"{$nextRow['litpic']}\" alt=\"{$nextRow['title']}\"/></a> ";
            }
            else
            {
                $this->PreNext['next'] = "下一篇：没有了 ";
                $this->PreNext['nextimg'] ="<a target='_blank' href='javascript:void(0)' alt=\"\"><img src=\"/templets/default/images/nophoto.jpg\" alt=\"对不起，没有下一图集了！\"/></a>";
            }
        }
        if($gtype=='pre')
        {
    ...
    function GetPagebreakDM($totalPage, $nowPage, $aid)
    {
        ...
        $PageList = "<a target='_blank'>共".$totalPage."页: </a>";
        $nPage = $nowPage-1;
        $lPage = $nowPage+1;
        if($nowPage==1)
        {
            $PageList.="<a target='_blank' href='#'>向前</a>";
        }
        else
        {
            if($nPage==1)
            {
                $PageList.="<a target='_blank' href='view.php?aid=$aid'>向前</a>>";
                if($cfg_rewrite == 'Y')
                {
            ...
            {
                $PageList.="<a target='_blank' href='view.php?aid=$aid&pageno=$nPage'>向前</a>";
                if($cfg_rewrite == 'Y')
                {
        ...
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                if($nowPage!=1)
                {
                    $PageList.="<a target='_blank' href='view.php?aid=$aid'>1</a>";
                    if($cfg_rewrite == 'Y')
                    {
                        $PageList = preg_replace("#.php\?aid=(\d+)#i", '-\\1-1.html', $PageList);
                    }
                ...
                {
                    $PageList.="<span class=\"now\"><a target='_blank'>1</a></span>";
                }
            }
            else
            {
                ...
                if($nowPage!=$i)
                {
                    $PageList.="<a target='_blank' href='view.php?aid=$aid&pageno=$i'>".$n."</a>";
                    if($cfg_rewrite == 'Y')
                    {
                        $PageList = str_replace(".php?aid=", "-", $PageList);
                        $PageList =  preg_replace("#&pageno=(\d+)#i", '-\\1.html', $PageList);
                ...
                {
                    $PageList.="<span class=\"now\"><a target='_blank' href='#'>{$n}</a></span>";
                }
            }
        }
        if($lPage <= $totalPage)
        {
            $PageList.="<a target='_blank' href='view.php?aid=$aid&pageno=$lPage'>向后</a>";
            if($cfg_rewrite == 'Y')
            {
                $PageList = str_replace(".php?aid=", "-", $PageList);
                $PageList =  preg_replace("#&pageno=(\d+)#i", '-\\1.html', $PageList);
        ...
        {
            $PageList.= "<a target='_blank' href='#'>向后</a>";
        }
        return $PageList;
    }
    ...
    function GetPagebreak($totalPage, $nowPage, $aid)
    {
	 $baseUrl = $GLOBALS['cfg_mip'].ereg_replace('{cmspath}',$GLOBALS['cfg_cmspath'],$this->Fields['typedir']);
        ...
        if($nowPage==1)
        {
            $PageList.="<a target='_blank' href='".$baseUrl."/'>向前</a>";
        }
        else
        {
            if($nPage==1)
            {
                $PageList.="<a target='_blank' href='".$baseUrl."/".$this->NameFirst.".".$this->ShortName."'>向前</a>";
            }
            else
            {
                $PageList.="<a target='_blank' href='".$baseUrl."/".$this->NameFirst."_".$nPage.".".$this->ShortName."'>向前</a>";
            }
        }
        for($i=1;$i<=$totalPage;$i++)
        {
            if($i==1)
            {
                if($nowPage!=1)
                {
                    $PageList.="<a target='_blank' href='".$baseUrl."/".$this->NameFirst.".".$this->ShortName."'>1</a>";
                }
                else
                {
                    $PageList.="<span class=\"now\">1</span>";
                }
            }
            else
            {
                ...
                if($nowPage!=$i)
                {
                    $PageList.="<a target='_blank' href='".$baseUrl."/".$this->NameFirst."_".$i.".".$this->ShortName."'>".$n."</a>";
                }
                else
                {
                    $PageList.="<span class=\"now\">{$n}</span>";
                }
            }
        }
        if($lPage <= $totalPage)
        {
            $PageList.="<a target='_blank' href='".$baseUrl."/".$this->NameFirst."_".$lPage.".".$this->ShortName."'>向后</a>";
        }
        else
        {
            $PageList.= "<a target='_blank' href='".$baseUrl."/' class=\"nextpage\">向后</a>";
        }
        return $PageList;
    }
    ...
    function GetPageTitlesDM($styleName, $pageNo)
    {
        ...
        if($styleName=='link')
        {
            ...
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
                    $revalue .= "<a target='_blank' href='view.php?aid=$aid&pageno=$i'>$v</a> \r\n";
                }
                else
                {
                    if($pageNo==$i)
    ...
    function GetPageTitlesST($styleName, $pageNo)
    {
        ...
        if($styleName=='link')
        {
            ...
            foreach($this->SplitTitles as $k=>$v)
            {
                if($i==1)
                {
                    $revalue .= "<a target='_blank' href='".$this->NameFirst.".".$this->ShortName."'>$v</a> \r\n";
                }
                else
                {
                    if($pageNo==$i)
                    ...
                    {
                        $revalue .= "<a target='_blank' href='".$this->NameFirst."_".$i.".".$this->ShortName."'>$v</a> \r\n";
                    }
                }
                $i++;
            }
    ...
    /**
     * 高亮问题修正, 排除alt title <a target='_blank'></a>直接的字符替换
     *
     * @param string $kw
     * @param string $body
     * @return string
    ...
    function ReplaceKeyword($kw,&$body)
    {
        ...
        //暂时屏蔽超链接
        $body = preg_replace("#(<a target='_blank'(.*))(>)(.*)(<)(\/a>)#isU", '\\1-]-\\4-[-\\6', $body);
        ...
        foreach($kws as $k)
        {
            ...
            if($k!="")
            {
                ...
                if(is_array($myrow))
                {
                    ...
                    $kaarr[] = "<a target='_blank' href='{$myrow['rpurl']}'><u>$k</u></a>";
                }
                $i++;
            }
        }
        */
        ...
        while($row = $this->dsql->GetArray())
        {
            ...
            $karr[] = $key;
            $kaarr[] = "<a target='_blank' href='$key_url' target='_blank'><u>$key</u></a>";
        }
        ...
        //恢复超链接
        $body = preg_replace("#(<a target='_blank'(.*))-\]-(.*)-\[-(\/a>)#isU", '\\1>\\3<\\4', $body);
        return $body;
    }
